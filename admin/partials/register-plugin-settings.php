...
function _modper_register_plugin_settings(){
    ...
    add_settings_field(
        'slug',                                       // Identificador de la sección de opciones.
        __('Slugs', 'modificaciones_personalizadas'), // Titulo
        '_modper_setting_slug_callback',              // Función Callback que imprime la descripción de la sección
        'modificaciones_personalizadas',              // Página donde se mostraran las opciones. 
    ...
    add_settings_field(
        'valores',
        __('Valores', 'modificaciones_personalizadas'),
        '_modper_setting_valores_callback',
        'modificaciones_personalizadas',
        'modper_general_settings_section',
        array(
            'label_for' => 'modper_valores'
        )     
    );
...
function _modper_print_section_info($args){
    ...
    ?>
        <p id="<?php echo esc_attr($args['id']); ?>"><?php esc_html_e('Ingrese los slugs y los valores de los campos que desea rellenar y ocultar del frontend.', 'modificaciones_personalizadas'); ?>
        </p>
    <?php
 ...
 * @param array $args
 */
function _modper_setting_slug_callback($args){
    ...
    ?>
        <textarea class="w-100 regular-text" placeholder="_price,_stock,_manage" id="<?php echo esc_attr($args["label_for"]); ?>" name="modper_options[<?php echo esc_attr($args["label_for"]); ?>]" rows="5" cols="50" required>
            <?php echo isset($options[$args["label_for"]]) ? (sanitize_text_field(esc_attr($options[$args["label_for"]]))) : (''); ?>
        </textarea>
        <p class="description" id="slug-description">Slug de cada campo en el orden de aparición, separados por coma.</p>
    <?php
 ...
 * @param array $args
 */
function _modper_setting_valores_callback($args){

    $options = get_option('modper_options');

    ?>
        <textarea class="w-100 regular-text" placeholder="1.90,5,yes" id="<?php echo esc_attr($args["label_for"]); ?>" name="modper_options[<?php echo esc_attr($args["label_for"]); ?>]" rows="5" cols="50" required>
            <?php echo isset($options[$args["label_for"]]) ? (sanitize_text_field(esc_attr($options[$args["label_for"]]))) : (''); ?>
        </textarea>
        <p class="description" id="valores-description">Valor de cada campo en el orden de aparición, separados por coma.</p>
    <?php
